...
class PreflightControllerTest {
    ...
    void anAmbiguousNameIs422WithCandidates() throws Exception {
        preflight(shareBody("{\"raw\": \"travel\"}"))
                .andExpect(status().isUnprocessableEntity())
                .andExpect(jsonPath("$.code").value("AMBIGUOUS_ENTITY"))
                .andExpect(jsonPath("$.step").value(1))
                .andExpect(jsonPath("$.param").value("folder_id"))
                .andExpect(jsonPath("$.candidates[*].label", contains("Travel plans", "Travel receipts")))
                .andExpect(jsonPath("$.candidates[0].id").value("8"))
