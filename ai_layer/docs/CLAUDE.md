...
Everything lives under `ai_layer/`. Folders marked with a phase do not exist yet; they are created by that phase. `STRUCTURE.md` explains every file.
...
2. **Resolve** every name, with scope in the SQL (`NOT_FOUND`, `AMBIGUOUS_ENTITY` with candidates).
