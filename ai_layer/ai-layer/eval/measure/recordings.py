...
model, a hash of the exact system prompt, and the raw answer for each sentence: before validation,
so a changed validator is measured against the same answers.
...
class Recordings:
    ...
    def __init__(
        ...
        *,
        ...
        self._header = {"model": model, "system_prompt_sha256": _sha256(system)}
        ...
        if stored is not None and {k: stored.get(k) for k in self._header} == self._header:
            ...
            reason = "no recording" if stored is None else "a recording for another model or prompt"
