...
def request(system: str = SYSTEM) -> ModelRequest:
    return ModelRequest(model="m", system=system, prompt="p", schema={}, purpose="plan")
...
async def test_replay_refuses_a_missing_answer_or_another_prompt(tmp_path: Path) -> None:
    ...
    with pytest.raises(MissingRecordingError, match="another model or prompt"):
