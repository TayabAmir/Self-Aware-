...
| `spi/EntityResolver.java`, `EntityMatch.java`, `Lookup.java`, `LookupField.java`                                     | What the host app implements for each entity type: a `Lookup` in (the user's words and the parts the plan named them with), every matching record the user may see out, each with an id, a label and what tells it apart; `lookup()`, what it searches by in plain words, and `fields()`, the parts it searches by (decision 78)                                                                                                                                                                                     |
...
| `metadata/ParamMetadata.java`, `ParamType.java`                                                                      | One input: name, type (string, integer, decimal, boolean, date), list or not, required, meaning, resolver, label, lookup (what the resolver searches by), lookup fields (its parts), allowed values, default                                                                                                                                                                                                                |
...
| `plan_validator.py` | `validate_plan`: ids in the metadata, the allow-list and the candidates; parameters declared, not duplicated, required ones present; forms, types, allowed values, real dates; names and amounts quoted from the sentence, each lookup part on its own; a lookup's parts declared, filled and naming a record; earlier-step facts; step count; versions stamped from the metadata |
...
**78. A plan names the parts of a lookup, instead of one line of words.** "Record payment of 2000 for Hasan Ali in
Class 5 Blue received through cash today" used to reach the backend as one blob the resolver had to take apart: drop
the words on a filler list, look for anything month-shaped, treat the rest as a name, and demand that every one of
those words appears in the record. Any extra word (and, at times, the whole sentence) matched nothing.

- **The resolver declares its parts.** `EntityResolver.fields()` returns `LookupField`s: for an invoice
  *student_name, invoice_no, month, year, class, section*, each with its meaning and whether it alone can name a
  record. They are published with every parameter that uses the resolver, so a change re-versions those capabilities
  and the AI layer re-syncs.
- **The plan carries them.** A looked-up parameter sends `lookup` (the parts) beside `raw` (the user's words, kept for
  the confirmation text and the audit trail). `resolve()` takes a `Lookup`, and each part goes to the column it names:
  the name to the name, the class and section to the placement, the month to the billing period. A plan that sends
  only `raw` still works, so nothing broke while the AI layer caught up.
- **Both sides refuse a bad lookup.** The backend rejects a part the resolver never declared, an empty one, and a
  lookup with nothing that can name a record (`INVALID_PLAN`); the validator does the same before the plan is sent,
  and checks every part is the user's own words.
- **Two prompt rules the first measurement forced.** The planner filled a year the user never wrote ("paid 9,000 for
  October" gained `year: 2026`), so it is now told never to work a part out. And it listed a parameter as missing
  while filling part of it, which the validator refused: that turned out to be the planner being right ("the class is
