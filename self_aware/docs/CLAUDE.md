...
**The planner** (model call 2) gets the sentence as typed, the intents labelled "a retrieval aid, not the plan", today's date in the school's time zone, and the candidates with their parameters. A parameter with a resolver is marked `looked_up_from_words`, with `looked_up_by_parts`: the parts its resolver declares (`EntityResolver.fields()`), each with its meaning, and `parts_that_name_the_record`, the ones that can name a record alone. The planner answers with `lookup`, one entry per part the user actually said, each in the user's own words and never worked out (decision 78); a resolver that declares no parts still gets `looked_up_by` prose and one line of `words`. The same text tells the user what to type when a name is not found. Version strings are never shown.

**The capability chooser** (on by default when a TypeSafe key is set, `AI_LAYER_CHOOSER_ENABLED`) sits between retrieval and the planner, in `app/choosing/`. TypeSafe's Jev (`jev-1.13.0`, pinned) gets one choice question per English intent, over the candidates plus "none", each described by what it does and needs; it answers with a probability per option and writes no text. The planner then sees only Jev's shortlist, and the validator holds it to that. An empty shortlist is a refusal; if Jev cannot be reached, the planner sees every candidate. `make measure-jev` compares both ways (README decision 72).
